...
const scriptTotest = "./createOrUpdateVersion.sh";
...
const dockerCallParameters = [
  ...
  ["uses create-or-update-version", /pact-cli.*broker create-or-update-version/],
  [
    "sets the participant",
    new RegExp(
      `create-or-update-version.*--pacticipant ${mandatoryVars.application_name}`
    ),
  ],
  [
    "sets version",
    new RegExp(`create-or-update-version.* --version ${mandatoryVars.version}`),
  ],
  ["sets branch", /create-or-update-version.* --branch .*/],
];
...
describe("createTag", () => {
  ...
  it("uses specified VERSION", () => {
    ...
    assert.match(
      ...
      /create-or-update-version.* --version 123/
    );
  });
